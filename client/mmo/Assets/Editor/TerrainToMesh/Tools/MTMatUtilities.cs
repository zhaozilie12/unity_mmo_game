namespace MightyTerrainMesh
{
    ...
    public static class MTMatUtils
    {
        private static Texture2D ExportAlphaMap(string path, string dataName, Terrain t, int matIdx)
        {
            ...
            {
                File.SetAttributes(alphaMapSavePath, FileAttributes.Normal);
        ...
        private static void SaveMixMaterail(string path, string dataName, Terrain t, int matIdx, int layerStart, string shaderName, List<string> assetPath)
        {
            ...
            tMat.SetTexture("_MainTex", alphaMap);
            for (int l = layerStart; l < layerStart + 4 && l < t.terrainData.terrainLayers.Length; ++l)
            {
                ...
                // }
                if (t.materialType == Terrain.MaterialType.Custom && t.materialTemplate != null && t.materialTemplate.shader.name == "H3D/Scene/Homeland/Terrain")
                {
                    if(idx == 0)
                    {
                        tMat.SetFloat("_Weight", t.materialTemplate.GetFloat("_Weight"));
                        tMat.SetTexture("_DetailColorNoise", t.materialTemplate.GetTexture("_DetailColorNoise"));
                        tMat.SetFloat("_DetailNoise01Tiling", t.materialTemplate.GetFloat("_DetailNoise01Tiling"));
                        tMat.SetFloat("_DetailNoise02Tiling", t.materialTemplate.GetFloat("_DetailNoise02Tiling"));
                        tMat.SetColor("_DetailColor01", t.materialTemplate.GetColor("_DetailColor01"));
                        tMat.SetColor("_DetailColor02", t.materialTemplate.GetColor("_DetailColor02"));
                        tMat.SetColor("_ShadowColor", t.materialTemplate.GetColor("_ShadowColor"));
                        tMat.SetColor("_DarkColor", t.materialTemplate.GetColor("_DarkColor"));
                    }
                    tMat.SetColor(string.Format("_Material0{0}Col", idx + 1), t.materialTemplate.GetColor(string.Format("_Material0{0}Col", idx + 1)));
                    tMat.SetFloat(string.Format("_Material0{0}Tiling", idx + 1), t.materialTemplate.GetFloat(string.Format("_Material0{0}Tiling", idx + 1)));
                    tMat.SetFloat(string.Format("_Material0{0}Gloss", idx + 1), t.materialTemplate.GetFloat(string.Format("_Material0{0}Gloss", idx + 1)));
                    tMat.SetColor(string.Format("_SpecularCol0{0}", idx + 1), t.materialTemplate.GetColor(string.Format("_SpecularCol0{0}", idx + 1)));
                }
            }
        ...
        public static void SaveMixMaterials(string path, string dataName, Terrain t, List<string> assetPath)
        {
            ...
            if (matCount <= 0)
                return;
            //base pass
            SaveMixMaterail(path, dataName, t, 0, 0, "H3D/Scene/Homeland/Ground", assetPath);
